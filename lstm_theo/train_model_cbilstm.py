...
EPOCH = 100
PATIENCE = 3
model_unique_name = 'norm_lm_xy_conv1d4u5k_dense4_bilstm8_' + str(BATCH_SIZE) + '_' + str(EPOCH) + '_' + str(PATIENCE) #name convention: (name)_batchsize_epoch
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Bidirectional( tf.keras.layers.LSTM(8, return_sequences = True) ), #default param: 8
...
save_best_callback = [
    tf.keras.callbacks.ModelCheckpoint(
        filepath        = './best_model_cnn_bilstm_' + model_unique_name + '.h5', 
        save_best_only  = True, 
        monitor         = 'val_accuracy',
        mode            = 'max'),
    tf.keras.callbacks.CSVLogger('./history_cnn_bilstm_' + model_unique_name + '.csv'),
