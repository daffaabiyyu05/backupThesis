...
def GenerateDirectories(sDirektoriData):
    CreateDir(sDirektoriData)
    CreateDir(sDirektoriData+"\\original")
    CreateDir(sDirektoriData+"\\face-mp")
    CreateDir(sDirektoriData+"\\pos-landmark")
    CreateDir(sDirektoriData+"\\pos-pixel")
    CreateDir(sDirektoriData+"\\face-mp-crop")
    CreateDir(sDirektoriData+"\\face-mp-crop-64")
    CreateDir(sDirektoriData+"\\face-mp-crop-128")
    CreateDir(sDirektoriData+"\\face-mp-crop-256")
    CreateDir(sDirektoriData+"\\face-mp-crop-512")
    CreateDir(sDirektoriData+"\\face-mp-crop-1024")
    CreateDir(sDirektoriData+"\\face-mp-append-300")
    print("Directories created for: " + sDirektoriData)
...
def parseVideo(capRecordFile, destDirectory="", viewImage = False):
    if capRecordFile == 0:
    ...
    if frame_per_second > 50:
        ...
        secondDirectory = destDirectory + nameSecondDir
        GenerateDirectories(secondDirectory)
    ...
    while cap.isOpened():
        ...
        if not doubleFrame: #singular frame
            dataGenerator(Counter, image, faceImg, landmarkPos, pixelPos, cropImg, sDirektoriData)
        else:
            if Counter%2 == 0: #even frames
                dataGenerator(Counter/2, image, faceImg, landmarkPos, pixelPos, cropImg, sDirektoriData)
            else: #odd frames
                dataGenerator((Counter+1)/2, image, faceImg, landmarkPos, pixelPos, cropImg, secondDirectory)
...
#train nodes
if not test_mode:
    first_node = [11, 15, 12, 12, 12, 12]
    last_node = [11, 15, 12, 12, 12, 12]
    BasePath = "D:\\Thesis\\data\\train\\"
#test nodes
else:
    #first_node = [11, 15, 12, 12, 12, 12]
    first_node = [11, 15, 12, 12, 12, 12]
    last_node = [15, 19, 15, 15, 14, 14]
    BasePath = "D:\\Thesis\\data\\test\\"
...
for TargetLetter in classes:
    ...
    base_letter_path = BasePath + TargetLetter + "\\"
    ...
    for path in sorted(glob(image_path + "*.*")):
        ...
        sDirektoriData = base_letter_path + sNamaDirektori 
        GenerateDirectories(sDirektoriData)
        print("Processing video: " + path)
        parseVideo(path, destDirectory=base_letter_path)
